...
/**
 * Orthovaria's own clinical Client endpoints. Thin delegation to the same
 * ClientQueryService dental's Api\ClientController uses. For index() and a
 * non-doctor's store(), the specialty is hardcoded to orthopedics rather than
...
class ClientController extends Controller
{
    ...
    {
        $clients = $this->clientQuery->list($request->user(), Specialty::ORTHOPEDICS, $request->validated());
    ...
    public function store(StoreClientRequest $request)
    {
        ...
        if ($actingUser->is_doctor) {
            ...
            $this->enrollment->ensureEnrolledForSpecialty($client, $orthopedics, $actingUser);
        }
