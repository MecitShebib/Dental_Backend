...
class AiConversationTest extends TestCase
{
            ...
            'https://api.openai.com/v1/chat/completions' => Http::response([
                'choices' => [
                    ['message' => ['content' => json_encode([
                        'diagnosis_summary' => 'Cosmetic consultation.',
                        'sessions' => [
                            [
                                'day_offset' => 0,
                                'duration_minutes' => 30,
                                'session_description' => 'Initial cosmetic consultation.',
                                'procedures' => [
                                    ['procedure_code' => 'cosmetic_consultation', 'notes' => null],
                                ],
                            ],
                        ],
                    ])]],
    ...
    public function test_it_generates_a_plan_using_the_procedure_vocabulary_not_teeth(): void
    {
        ...
        $response = $this->postJson("/api/cosmetic/clients/{$client->id}/ai-treatment-plan/generate", [
            'text' => 'Patient wants a botox consultation.',
        ])->assertOk();

        $response->assertJsonPath('data.diagnosis_summary', 'Cosmetic consultation.')
            ->assertJsonCount(1, 'data.sessions')
            ->assertJsonPath('data.sessions.0.procedures.0.procedure_code', 'cosmetic_consultation')
    ...
    public function test_it_confirms_a_plan_and_creates_an_appointment_enrolled_as_cosmetic(): void
    {
        $doctor = $this->doctorWithFullWeekSchedule();
        Sanctum::actingAs($doctor);
        $client = $this->makeClient($doctor->company);
        ...
        $response = $this->postJson("/api/cosmetic/clients/{$client->id}/ai-treatment-plan/confirm", [
            'sessions' => [
                [
                    ...
                    'charge_items' => [
                        ['description' => 'Cosmetic Consultation', 'amount' => 300],
                    ],
                ],
            ],
        ]);
