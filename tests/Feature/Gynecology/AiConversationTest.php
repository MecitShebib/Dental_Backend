...
class AiConversationTest extends TestCase
{
            ...
            'https://api.openai.com/v1/chat/completions' => Http::response([
                'choices' => [
                    ['message' => ['content' => json_encode([
                        'diagnosis_summary' => 'Routine prenatal visit.',
                        'sessions' => [
                            [
                                'day_offset' => 0,
                                'duration_minutes' => 30,
                                'session_description' => 'Prenatal checkup and ultrasound.',
                                'procedures' => [
    ...
    public function test_it_generates_a_plan_using_the_procedure_vocabulary_not_teeth(): void
    {
        ...
        $response = $this->postJson("/api/gynecology/clients/{$client->id}/ai-treatment-plan/generate", [
            'text' => 'Patient is 20 weeks pregnant, routine visit.',
        ])->assertOk();

        $response->assertJsonPath('data.diagnosis_summary', 'Routine prenatal visit.')
            ->assertJsonCount(1, 'data.sessions')
            ->assertJsonPath('data.sessions.0.procedures.0.procedure_code', 'prenatal_checkup')
    ...
    public function test_it_confirms_a_plan_and_creates_an_appointment_enrolled_as_gynecology(): void
    {
        $doctor = $this->doctorWithFullWeekSchedule();
        Sanctum::actingAs($doctor);
        $client = $this->makeClient($doctor->company);
