...
class AiConversationTest extends TestCase
{
            ...
            'https://api.openai.com/v1/chat/completions' => Http::response([
                'choices' => [
                    ['message' => ['content' => json_encode([
                        'diagnosis_summary' => 'Orthopedic rehab follow-up.',
                        'sessions' => [
                            [
                                'day_offset' => 0,
                                'duration_minutes' => 30,
                                'session_description' => 'Orthopedic assessment session.',
                                'procedures' => [
                                    ['procedure_code' => 'ortho_assessment', 'notes' => null],
                                ],
                            ],
                        ],
                    ])]],
    ...
    protected function doctorWithFullWeekSchedule(): User
    {
        ...
        $doctor = User::factory()->create(['is_doctor' => true, 'specialty_id' => $orthopedics->id]);
    ...
    public function test_it_generates_a_plan_using_the_procedure_vocabulary_not_teeth(): void
    {
        ...
        $response = $this->postJson("/api/orthopedics/clients/{$client->id}/ai-treatment-plan/generate", [
            'text' => 'Patient has a knee injury, needs assessment.',
        ])->assertOk();

        $response->assertJsonPath('data.diagnosis_summary', 'Orthopedic rehab follow-up.')
            ->assertJsonCount(1, 'data.sessions')
            ->assertJsonPath('data.sessions.0.procedures.0.procedure_code', 'ortho_assessment')
    ...
    public function test_it_confirms_a_plan_and_creates_an_appointment_enrolled_as_orthopedics(): void
    {
        $doctor = $this->doctorWithFullWeekSchedule();
        Sanctum::actingAs($doctor);
        $client = $this->makeClient($doctor->company);

        $response = $this->postJson("/api/orthopedics/clients/{$client->id}/ai-treatment-plan/confirm", [
            'sessions' => [
                [
                    'date' => now()->addDay()->toDateString(),
                    'start_time' => '10:00',
                    'duration_minutes' => 30,
                    'session_description' => 'Orthopedic assessment.',
                    'charge_items' => [
                        ['description' => 'Orthopedic Assessment', 'amount' => 500],
                    ],
                ],
            ],
        ]);
