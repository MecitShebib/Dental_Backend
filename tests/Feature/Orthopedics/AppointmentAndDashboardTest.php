...
class AppointmentAndDashboardTest extends TestCase
{
    ...
    public function test_appointments_and_dashboard_stats_are_scoped_to_orthopedics_even_without_a_specialty_query_param(): void
    {
        ...
        $orthoDoctor = User::factory()->create(['company_id' => $company->id, 'is_doctor' => true, 'specialty_id' => $orthopedics->id]);
        $dentalDoctor = User::factory()->create(['company_id' => $company->id, 'is_doctor' => true, 'specialty_id' => $dental->id]);

        $this->makeAppointmentFor($company, $orthoDoctor, 'Ortho Appt Patient');
        $this->makeAppointmentFor($company, $dentalDoctor, 'Dental Appt Patient');

        Sanctum::actingAs($manager);

        $apptResponse = $this->getJson('/api/orthopedics/appointments');
        $apptResponse->assertOk();
        $names = collect($apptResponse->json('data'))->pluck('client_name');
        $this->assertTrue($names->contains('Ortho Appt Patient'));
        $this->assertFalse($names->contains('Dental Appt Patient'));

        $statsResponse = $this->getJson('/api/orthopedics/dashboard/stats?date_from='.now()->toDateString().'&date_to='.now()->toDateString());
